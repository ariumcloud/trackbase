-- A provider can resend the same webhook after the first delivery was already
-- marked as processed.  Keep the operation idempotent, but allow an enriched
-- replay to repair country and attribution fields that were missing in the
-- first normalization.
