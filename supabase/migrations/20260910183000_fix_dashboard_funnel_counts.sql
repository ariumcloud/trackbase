-- Keep the funnel stages semantically correct. Scroll milestones and CTA
-- visibility are stored as `cta` for legacy compatibility, but are not CTA
-- clicks. A checkout click is also a CTA click and belongs in that stage.
...
begin
  ...
  select
    ...
    coalesce(
      count(*) filter (
        where event_type = 'cta'
          and coalesce(attribution->>'action', '') in ('', 'cta_click')
      )
