-- Store the actual budget returned by the Meta Marketing API. Values are in minor units.
alter table public.utm_ad_entities
  add column if not exists budget_minor numeric,
  add column if not exists budget_currency text,
  add column if not exists budget_type text check (budget_type in ('daily','lifetime'));

create or replace function public.utm_commit_meta_sync(p_integration uuid,p_since date,p_until date,p_entities jsonb,p_insights jsonb) returns void language plpgsql set search_path='' as $$
 ...
 insert into public.utm_ad_entities(workspace_id,integration_id,external_id,kind,parent_id,name,status,budget_minor,budget_currency,budget_type)
 select w,p_integration,r.external_id,r.kind,r.parent_id,r.name,r.status,r.budget_minor,r.budget_currency,r.budget_type
 from jsonb_to_recordset(p_entities) as r(external_id text,kind text,parent_id text,name text,status text,budget_minor numeric,budget_currency text,budget_type text);
